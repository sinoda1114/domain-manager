...
export function DomainNameEditor({ domainId, displayName, disabled = false }: { domainId: string; displayName: string | null; disabled?: boolean }) {
  ...
  const cancel = () => { setValue(displayName ?? ""); setError(""); setEditing(false); };
  const save = async () => {
    ...
    try {
      ...
      setValue(nextValue); setEditing(false); router.refresh();
    } catch (caught) { setError(caught instanceof Error ? caught.message : "サービス名を更新できませんでした。"); }
    finally { setSaving(false); }
  };
  if (!editing) return <div className="domain-name"><small>サービス名</small><strong>{displayName ?? "未設定"}</strong><button type="button" onClick={() => setEditing(true)} disabled={disabled}>変更</button></div>;
