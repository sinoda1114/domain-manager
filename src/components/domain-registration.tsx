...
export function DomainRegistration({ rootDomain, targets, compact = false }: { rootDomain: string; targets: ProviderTarget[]; compact?: boolean }) {
  ...
  const displayNameError = displayName.trim() ? "" : "サービス名を入力してください。";
  ...
  const targetLabel = (candidate: ProviderTarget) => candidate.repositoryName ? `${candidate.name} · ${candidate.repositoryName}` : candidate.name;
  ...
  const changeProvider = (value: Provider) => { setProvider(value); setTargetId(targets.find((candidate) => candidate.provider === value)?.id ?? ""); setTargetQuery(""); setTargetPickerOpen(false); setSuggestions([]); setSelectedSuggestion(""); setRequestError(""); };
  const suggest = async () => {
    if (!target) return;
  ...
  const openConfirmation = () => {
    if (error || displayNameError || !target) return;
  ...
  return <section className={compact ? "panel registration registration-compact" : "panel registration"} id="register">
    ...
    <div className={compact ? "register-grid compact-grid" : "register-grid"}><div className="form-area">
      ...
      <div className="field target-field"><label htmlFor="target">公開先プロジェクト / リポジトリ</label><div className="target-picker" onBlur={() => window.setTimeout(() => setTargetPickerOpen(false), 120)}><div className="search-field"><input id="target" type="search" role="combobox" autoComplete="off" value={targetPickerOpen ? targetQuery : target ? targetLabel(target) : ""} onFocus={() => { setTargetPickerOpen(true); setTargetQuery(""); }} onChange={(event) => { setTargetQuery(event.target.value); setTargetPickerOpen(true); }} onKeyDown={(event) => { if (event.key === "Escape") setTargetPickerOpen(false); }} placeholder="公開先を検索・選択" aria-expanded={targetPickerOpen} aria-controls="target-options" aria-autocomplete="list" disabled={availableTargets.length === 0} />{targetPickerOpen && targetQuery && <button type="button" className="search-clear" onMouseDown={(event) => event.preventDefault()} onClick={() => setTargetQuery("")} aria-label="検索語を消去">×</button>}</div>{targetPickerOpen && <div className="target-options" id="target-options" role="listbox" aria-label="公開先の候補">{availableTargets.length === 0 ? <p>利用できる公開先がありません。</p> : filteredTargets.length === 0 ? <p>一致する公開先がありません。</p> : filteredTargets.map((item) => <button type="button" role="option" aria-selected={item.id === targetId} key={item.id} onMouseDown={(event) => event.preventDefault()} onClick={() => { setTargetId(item.id); setTargetQuery(""); setTargetPickerOpen(false); setSuggestions([]); setSelectedSuggestion(""); }}><strong>{item.name}</strong>{item.repositoryName && <small>{item.repositoryName}</small>}</button>)}</div>}</div>{target?.repositoryName && <span className="hint">リポジトリ: {target.repositoryName}</span>}</div>
      ...
      <div className="draft-summary"><div><span>反映するサブドメイン</span><strong>{fqdn}</strong></div><div><span>サービス名</span><strong>{displayName.trim() || "未入力"}</strong></div><div><span>公開先</span><strong>{target ? `${providerLabels[provider]} / ${target.name}` : "未選択"}</strong>{target?.sourceUrl && <a className="source-link" href={target.sourceUrl} target="_blank" rel="noreferrer">実体URL ↗</a>}</div><div><span>自動削除</span><strong>{deleteAt ? formatJst(toJstIso(deleteAt) ?? deleteAt) : "設定なし"}</strong></div><small>確認後に反映</small></div><div className="form-actions"><span className="dry-run">反映前に内容を確認できます</span><button className="plan-button" type="button" disabled={Boolean(error) || Boolean(displayNameError) || !target || submitting} onClick={openConfirmation}>反映内容を確認</button></div>
    </div></div>
